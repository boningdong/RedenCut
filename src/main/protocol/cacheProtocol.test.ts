...
async function projectRoot() {
  const root = await mkdtemp(join(tmpdir(), 'redencut-protocol-'))
...
describe('managed cache protocol', () => {
  it('serves a validated PCM artifact through the real bounded file adapter', async () => {
    ...
    const response = await handler(
      new Request(`redencut://cache/${SOURCE_ID}/pcm`, { headers: { Range: 'bytes=0-3' } }),
    )
  ...
  it.each([
    'redencut://other/00000000-0000-4000-8000-000000000001/pcm',
    'redencut://cache/not-a-uuid/pcm',
    `redencut://cache/${SOURCE_ID}/waveform/512`,
    `redencut://cache/${SOURCE_ID}/../../project.json`,
  ...
  it('rejects missing and oversized byte ranges before touching the filesystem adapter', async () => {
    ...
    expect((await handler(new Request(`redencut://cache/${SOURCE_ID}/pcm`))).status).toBe(416)
    expect(
      (
        await handler(
