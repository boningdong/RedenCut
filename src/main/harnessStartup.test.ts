...
test('sets isolated userData, sessionData and temp paths without touching other files', () => {
  const root = mkdtempSync(join(tmpdir(), 'podcut-startup-test-'))
  roots.push(root)
  ...
  expect(
    configureHarnessStartup(
      ...
      {
        PODCUT_HARNESS_RUN_DIRECTORY: runDirectory,
        PODCUT_HARNESS_RUN_ID: runId,
      },
    ),
  ).toBe(true)
  expect(paths).toEqual({
  ...
  (runDirectory) => {
    expect(() =>
      configureHarnessStartup(
        ...
        {
          PODCUT_HARNESS_RUN_DIRECTORY: runDirectory,
          PODCUT_HARNESS_RUN_ID: '318932a4-3c50-4a27-a9f8-0288a189b5d7',
        },
      ),
