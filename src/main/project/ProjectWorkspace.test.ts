...
describe('ProjectWorkspace', () => {
  ...
  it('keeps a committed Save As result and records only its exact leftover destination backup', async () => {
    ...
    expect(saved.root).toBe(destination)
    expect(await readFile(join(destination, 'project.json'), 'utf8')).toContain('"version": 3')
  ...
  it('retains the previous saved package when Save As switches to a new package', async () => {
    ...
    expect(await readFile(join(first, 'project.json'), 'utf8')).toContain('"version": 3')
    expect(await readFile(join(second, 'project.json'), 'utf8')).toContain('"version": 3')
