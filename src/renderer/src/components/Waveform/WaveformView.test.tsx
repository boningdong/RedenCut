...
describe('WaveformView managed providers', () => {
  ...
  it('shows linked recordings read-only and collapses their lanes', () => {
    ...
    expect(linkButton.querySelector('svg')?.getAttribute('stroke-width')).toBe('1.7')
    expect(linkButton.querySelector('svg')?.getAttribute('stroke-linecap')).toBe('round')
    fireEvent.contextMenu(screen.getByLabelText(`${mix.name} track actions`))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Hide linked recordings' }))
    expect(container.querySelector('[data-lane="child"]')).toBeNull()
    fireEvent.contextMenu(screen.getByLabelText(`${mix.name} track actions`))
    fireEvent.click(screen.getByRole('menuitem', { name: 'Show linked recordings' }))
