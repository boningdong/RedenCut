...
The `.redencut` bundle remains unchanged:
...
- `redencut://cache/<audioSourceId>/pcm` and waveform route validation;
...
The renderer Content Security Policy explicitly permits cache fetches with `connect-src 'self' redencut:`. The `redencut` scheme remains registered with `secure`, `supportFetchAPI`, and `stream` privileges, but it must not use Electron's `bypassCSP` privilege.
...
PCM or waveform provider
    -> redencut://cache URL plus bounded Range
...
The workflow must also confirm that renderer-originated `redencut://cache/...` requests reach the registered handler without CSP violations. A successful main-process diagnostic alone is insufficient because it bypasses the renderer policy boundary.

## Completion criteria

The repair is complete when the real file adapter returns exact standards-compliant `206` responses, PCM and waveform providers succeed through the protected protocol, the renderer CSP permits `redencut:` without `bypassCSP`, the supplied MP3 plays and seeks at late positions without cache transport errors, the repository quality gate passes, and no project-format or renderer-authority expansion has been introduced.
