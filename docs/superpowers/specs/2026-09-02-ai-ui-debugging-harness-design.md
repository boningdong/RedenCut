# RedenCut AI UI Debugging Harness
...
Build a local macOS-first harness that lets an AI start an isolated, visible RedenCut instance, operate its real UI, inspect evidence, restart it after code changes, and repeat the same workflow.
...
- A custom RedenCut Harness MCP server is the only AI-facing entry point.
...
The external server exposes a curated combination of `redencut_*` application tools and upstream UI tools.
...
| Tool                      | Contract                                                                                                     |
| ------------------------- | ------------------------------------------------------------------------------------------------------------ |
| `redencut_start`            | Start one isolated run from an allowed scenario/configuration; return run identity and readiness             |
| `redencut_status`           | Report run state, current generation, startup stage, available diagnostics, and errors                       |
| `redencut_restart`          | Explicitly rebuild/restart as requested, retaining the isolated project unless reset is explicitly requested |
| `redencut_stop`             | Stop the owned application with bounded cleanup and preserve artifacts                                       |
| `redencut_list_scenarios`   | Describe available fixtures and their dependency requirements                                                |
| `redencut_prepare_dialog`   | Register one typed, purpose-specific native-dialog response before the UI action                             |
| `redencut_read_diagnostics` | Read narrow product/run state without mutating editing state                                                 |
| `redencut_list_artifacts`   | List evidence for the selected run/generation                                                                |
...
Recovery must validate run ownership and process identity before acting, and must never stop the user's ordinary RedenCut process based on its name alone.
...
Gate A is a compatibility test, not proof that the full RedenCut workflows pass.
...
- Apply the lifecycle host to the real RedenCut empty state; fixture import and dialog-dependent workflows are not required yet.
...
### Gate C: real RedenCut boundaries

- Apply the same runtime to the real RedenCut build.
...
They verify RedenCut user workflows and reuse the harness runtime without moving product scenarios into the harness's own tests.
...
Successful runs demonstrate the infrastructure and the real RedenCut empty state; the September 3 Main-startup timeouts and subsequent September 5 revalidation are recorded below.
...
| B          | Real RedenCut readiness, isolated paths/locks, loopback debug listeners, lifecycle tools, explicit rebuild/restart, fresh-generation snapshots, logs/traces, ordinary disconnect/SIGTERM cleanup                                                            |
...
- Host SIGKILL cannot run cleanup handlers: `redencut_status` reports matching surviving Electron processes, but recovery remains manual and must revalidate process identity before termination.
