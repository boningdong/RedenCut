...
It defines the canonical transcript and acoustic-alignment model, local worker boundary, project persistence, selection resolution, atomic publication, invalidation, and integration with RedenCut's current project and job infrastructure.
...
Given one managed `AudioSource`, RedenCut produces and atomically publishes one coherent speech-analysis result containing:
...
- Compact speech references in `.redencut/project.json` and immutable canonical artifacts in `.redencut/speech/`.
...
SpeechAnalysisCoordinator (Electron main)
      ...
      └── atomic workspace publication
                    │
                    ▼
        .redencut/project.json ── SpeechArtifactRef
                                    │ path + SHA-256
                                    ▼
                 .redencut/speech/<audio-source-id>/
...
`defaultDisplayName` is the generated user-facing label, such as `Speaker 1`. A user who knows the participant may enter a real name through a `SpeakerLabelOverride`, but RedenCut does not infer or verify real-world identity from the voice. User edits never mutate the speaker ID, raw diarization label, turns, or attribution.
...
The large payload is stored at `.redencut/speech/<audioSourceId>/revision-<analysisRevisionId>.json`. `ProjectFile` stores only one compact reference per currently analyzed source:
...
On project load and after successful publication, RedenCut builds:
...
Engine results are adapter DTOs, not persisted project types. Main-process normalization assigns RedenCut IDs, converts units and times, validates provenance, and constructs the canonical artifacts.
...
- The RedenCut Python worker is the process host for alignment and diarization, not an engine or model itself.
...
The first version stores the following complete immutable payload in `.redencut/speech/<audioSourceId>/revision-<analysisRevisionId>.json`:
...
The compact `SpeechArtifactRef[]`, `SpeakerLabelOverride[]`, and non-destructive user edits remain in `.redencut/project.json`. These are all project data, not cache. Cleaning `.redencut/cache` must not remove `.redencut/speech`, canonical transcripts, acoustic edit units, speakers, display-name overrides, or user edits.
...
## 13. Integration assessment against current RedenCut
