...
This document is the program-level design for RedenCut's speech-intelligence work.
...
A general `PipelineStep<Input, Output>` framework is intentionally deferred. RedenCut first defines domain interfaces whose names and contracts express the actual capability required.
...
Scores initially rank suggestions; they are not presented as literal correctness probabilities without calibration on representative RedenCut data.
...
The first macOS release uses an independently installed local model worker. Electron does not embed Python or model runtimes, and RedenCut detects the worker and provides actionable installation guidance when it is unavailable.
...
Subproject plans define exact metrics. At program level, RedenCut tracks:
