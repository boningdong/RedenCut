...
**Goal:** Establish a green verification baseline, enforce the approved repository rules, apply isolated mechanical formatting, and remove only code and dependencies proven unused by the current PodCut implementation.
...
Do not add unrelated example domains. Add a PodCut example only when a real repository case materially clarifies a rule.
