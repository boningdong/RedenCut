...
- Add no project schema, manifest, IPC, preload, renderer-provider, or `.podcut` layout changes.
...
it('propagates a missing authorized artifact so the protocol can report 500', async () => {
  await expect(
    createFileRangeResponse(
      join(tmpdir(), 'podcut-definitely-missing-artifact.bin'),
      { start: 0, end: 1 },
      new AbortController().signal,
    ),
...
Also assert the scoped fetch bridge observed only `podcut://cache/` URLs and bounded `Range` headers.
...
- Leaves `createCacheProtocolHandler` and renderer-facing `podcut://cache` URLs unchanged.
