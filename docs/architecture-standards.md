...
- Auto Level analyzes the dry composed track (including Replace) in 100 ms RMS windows, then applies a smoothed, gated correction around its active-window median with a ±12 dB correction bound; it does not target a fixed integrated LUFS. Independently composed tracks retain independent references. Gating is amplitude-based, not speech recognition, and peak protection bounds samples rather than certified intersample peaks.
- Generate the processed waveform index during the final PCM write. Keep PCM streaming and retain only window statistics and bounded waveform levels in memory; do not rescan the completed PCM to build its waveform.
- Playback and waveform consumers share content-keyed preparation within a sender/workspace, with independent leases and cancellation. Gain, Volume and equivalent playback-mode labels do not invalidate identical sample composition. Retain at most two idle results within a 2 GiB budget; workspace teardown releases active and idle results.
- Export with Auto Level encodes the same prepared-PCM algorithm and applies manual Gain/Volume exactly once. Original media and legacy effect parameter serialization remain unchanged; legacy loudness-target/LRA fields no longer impose fixed-loudness processing.
- Waveform preparation reports measured completed/total frames in separate processing and waveform phases. Keep the previous waveform during replacement preparation and reject stale progress/completion after a track or workspace switch.
...
- Replacement presentation is derived from linked count, selected source identities and available pixel width, never persisted as routing truth. Replacement captions share the top clip-label strip. Up to three selected sources show separate source-colored waveform rows filling the area below, including after processing completes. More than three selected sources retain the combined waveform of the actual processed Mix with source-color dots and a count. These rows visualize routed source provenance, with master Gain applied; they are not a decomposition of nonlinear Mix effects. Prepared composite waveforms draw ordinary master intervals and replacements with more than three sources, using the shared CanvasWaveform renderer and output-time coordinates; and audio processing still applies to the actual complete Mix.
