...
# Optional existing app model fixtures are mounted read-only, without account/profile data.
if [ -n "${REDENCUT_TEST_MODEL_FIXTURE:-}" ]; then
  set -- --mount "type=bind,source=$REDENCUT_TEST_MODEL_FIXTURE,target=/test-models,readonly" "${REDENCUT_HARNESS_IMAGE:-redencut-harness:local}" "$@"
else
