...
Host source edits require closing and recreating the container; `podcut_restart` with `rebuild: true` rebuilds the same container snapshot, not newer host files.
...
`PODCUT_HARNESS_IMAGE` overrides the image tag and `PODCUT_CONTAINER_NAME` optionally gives a specific container name.
Containers have the label `dev.podcut.harness=container` for scoped inspection.
