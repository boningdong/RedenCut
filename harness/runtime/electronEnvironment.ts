export function electronEnvironment(source: NodeJS.ProcessEnv): Record<string, string> {
  ...
  for (const key of [
    'RIFFCUT_SPEECH_WORKER_ROOT',
    'RIFFCUT_SPEECH_WORKER_PYTHON',
    'RIFFCUT_SPEECH_MANIFEST',
    'RIFFCUT_SPEECH_MODEL_CACHE',
    'RIFFCUT_WHISPER_MODEL_DIR',
  ]) {
    const value = source[key]
    if (value !== undefined) env[key] = value
  }
  if (
    source.RIFFCUT_CONTAINER_AUDIO === '1' &&
    source.PULSE_SERVER === 'unix:/tmp/riffcut-audio/native' &&
    source.PULSE_SINK === 'riffcut_test'
  ) {
